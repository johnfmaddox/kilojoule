...
def test_check_solutions_legend_shows_only_kinds_present(hash_db):
    ...
    assert len(captured) == 2  # the result line, then the legend
    result_tex, legend_tex = (obj.data for obj in captured)
    assert r"\checkmark" in result_tex
    assert r"\checkmark" in legend_tex
    assert r"\approx" not in legend_tex
