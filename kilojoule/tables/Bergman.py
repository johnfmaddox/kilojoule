...
_transport_property_data_path = os.path.join(os.path.realpath(os.path.join(__file__,os.pardir,'Bergman Data')))
...
class Properties:
    def __init__(self, material=None, file=None, unit_system="kSI_K", verbose=False):
        ...
        for p in self.properties:
            prop_func = functools.partial(self._property_lookup,p,verbose=self.verbose)
    ...
    def _interp(self, dependent_property, independent_property, independent_value, verbose=False):
        ...
        if verbose: print(f'{dependent_property=}, {independent_property=}, {independent_value=}, {ind_units=}, {dep_units=}')
        result = Quantity(float(interp_func(independent_value.to(ind_units).magnitude)),dep_units)
        return result
