"""
    ...
    latex` + `xelatex`, working around problems in plain `jupyter
    nbconvert --to pdf`:
    ...
    2. Every ``Summary()`` state table (``QuantityTable.display()``, a
       pandas DataFrame rendered via ``display(HTML(...))``, so it only
       ever registers under the ``text/html`` mimetype) degrades into a
       flat list of numbers instead of a table -- ``text/html`` isn't
       even in nbconvert's LaTeX-exporter output-priority list, so it's
       invisible there and nbconvert falls back to the next
       representation available (``text/plain``) instead.
    3. A markdown cell's `<img src="attachment:NAME">` tag (a pasted image
       referenced as raw HTML rather than Markdown image syntax) vanishes
       from the PDF entirely -- pandoc drops raw HTML `<img>` tags as
       unsupported when converting to LaTeX, and even if it didn't,
       `\\includegraphics` can't load an `attachment:` URI as a file path
       anyway. See :func:`extract_cell_attachments`.
    4. A long equation runs off the edge of the page instead of wrapping --
       plain ``align`` only breaks where a literal ``\\\\`` says to. See
       :func:`convert_notebook_long_rows`.
    5. A Markdown list immediately following a paragraph, with no blank
       line between them, collapses into run-on text instead of becoming
       a proper list -- pandoc's markdown dialect requires that blank
       line. See :func:`ensure_blank_lines_before_lists`.
    6. A list hand-labeled ``(a)``, ``(b)``, ``(c)``, ... renders as a
       bullet list with a redundant literal "(a)" in the text, rather
       than a proper ``(a)``-labeled ``enumerate``. See
       :func:`convert_lettered_lists`.
    7. The PDF opens with nbconvert's generic title page. See
       :func:`remove_title_block`.

    Full details for each of the above are in
    :func:`~kilojoule.export.export_pdf`'s docstring, which this module's
...
# Aliased -- this module's functions overwhelmingly use `html` as a
# parameter name for raw HTML text (matching export._ATTACHMENT_IMG_RE's
# convention), which would shadow a plain `import html`.
import html as _html_stdlib

from .export import sanitize_notebook_outputs

# Matches a *whole* `<img ...>` tag referencing `attachment:NAME`, unlike
# export._ATTACHMENT_IMG_RE (which matches only through the closing quote
# of `src="attachment:NAME"`, leaving any trailing attributes -- e.g.
# `style="max-width:100%"` -- as stray literal text after a substitution
# in extract_cell_attachments below; harmless for
# export.resolve_cell_attachments, which only rewrites the `src` value in
# place and leaves the rest of the tag as valid HTML, but not here, where
# the whole tag is replaced with Markdown image syntax).
...
# ---------------------------------------------------------------------------
# Rewrite embedded HTML <table> blocks (in text/markdown or text/html
# outputs) as native LaTeX tabular blocks
# ---------------------------------------------------------------------------
...
def captioned_block(*parts):
    ...
    e.g. a caption and the table/figure it captions, in whichever order
    the caller wants them to appear) inside a centered ``minipage`` the
    full ``\\linewidth`` wide.

    A ``minipage`` is typeset as a single, indivisible box: if it doesn't
    fit in the remaining space on the current page, LaTeX pushes the
    *whole* box to the next page rather than ever splitting a caption
...
# Fallback caption for a table saved before kilojoule.organization added
# a real <caption> tag to QuantityTable.display()'s own HTML (see
# default_table_caption there) -- kept in sync with that default so an
# already-existing notebook re-exported today reads the same either way.
...
def html_table_to_latex(html):
    ...
    overflowing the page, centered on the page with an auto-numbered
    caption on top (via ``\\captionof{table}{...}`` from the `capt-of`
    package -- this isn't a floating ``table`` environment, so plain
    ``\\caption`` isn't available).

    The caption text is read from the table's own ``<caption>`` element
    if it has one (``QuantityTable.display()`` adds one, so the HTML and
    PDF exports of the same table always agree), falling back to
    :data:`_TABLE_CAPTION_FALLBACK` for a table saved before that existed.
    """
...
# ---------------------------------------------------------------------------
# Convert kilojoule.plotting.PropertyPlot.show()'s captioned
# <figure><img src="data:...">...<figcaption>...</figcaption></figure>
# into a centered, captioned LaTeX \includegraphics
# ---------------------------------------------------------------------------
...
def html_figure_to_latex(html, out_path):
    ...
    :meth:`kilojoule.plotting.PropertyPlot.show` displays a captioned plot
    -- to a centered LaTeX ``\\includegraphics`` with the caption below it
    (``\\captionof{figure}{...}`` from the `capt-of` package -- this isn't
    a floating ``figure`` environment, so plain ``\\caption`` isn't
    available), extracting the embedded image to a real file.

    :param html: raw HTML text containing (at least) one ``<figure>`` block
    :param out_path: file path to write the extracted image to (its
        suffix is corrected to match the image's real mimetype if needed)
    :returns: `(new_text, out_path)` with the figure replaced and the
        image written, using `out_path`'s corrected suffix -- or
        `(html, None)` unchanged if nothing recognizable was found (no
        `<figure>`, or none containing a `data:` URI `<img>`)
    """
...
def extract_cell_figures(nb, stem):
    """Apply :func:`html_figure_to_latex` to every code cell output
    containing a ``<figure>`` block (under ``text/html`` -- see
    :func:`fix_notebook_tables` for why that mimetype needs special
    handling for nbconvert's LaTeX exporter), landing the fixed LaTeX in
    ``text/markdown`` (creating it if absent) and writing the extracted
    image to a file next to the notebook (mutates `nb` in place).

    :param nb: parsed notebook dict (mutated in place)
    :param stem: base filename (no extension) extracted image files are
        prefixed with, so they sit alongside -- and are identifiable as
        belonging to -- the notebook being converted
    :returns: `(n_fixed, [Path, ...])` -- count of figures converted, and
        the files written (for the caller to clean up)
    """
    written = []
    n_fixed = 0
...
def extract_cell_attachments(nb, stem):
    ...
    image (already embedded as base64 in that cell's `attachments` dict,
    per the nbformat spec) to a real file next to the notebook, and rewrite
    that tag to reference the file instead (mutates `nb` in place): plain
    Markdown image syntax normally, or -- when the ``<img>`` is wrapped in
    a ``<figure>...<figcaption>...</figcaption>...</figure>`` (the same
    convention :meth:`kilojoule.plotting.PropertyPlot.show` uses) -- a
    centered, captioned LaTeX ``\\includegraphics`` instead (see
    :func:`html_figure_to_latex`, which this mirrors).

    pandoc -- which nbconvert's LaTeX exporter uses to turn markdown into
    LaTeX -- drops a raw HTML `<img>`/`<figure>` tag as unsupported raw
    HTML when the target is LaTeX, regardless of its `src`; an
    `attachment:` URI isn't something `\\includegraphics` could load as a
    file path either way. Rewriting to a real file lets pandoc (or, for a
    captioned figure, this function directly) turn it into a proper
    `\\includegraphics`, the same as any other embedded plot.

    :param nb: parsed notebook dict (mutated in place)
    :param stem: base filename (no extension) the extracted image files
        are prefixed with, so they sit alongside -- and are identifiable
        as belonging to -- the notebook being converted
    :returns: `(n_fixed, [Path, ...])` -- count of tags rewritten, and the
        files written (for the caller to clean up)
    """
    written = []
    n_fixed = 0
    for ci, cell in enumerate(nb.get("cells", [])):
        ...
        # <figcaption>...</figcaption>...</figure> wrapper into a
        # captioned embed first, then handle any remaining bare
        # <img src="attachment:...">.
...
# ---------------------------------------------------------------------------
# Long-equation wrapping: split a too-wide `align` row at `=` step
# boundaries and/or `+`/`-` term boundaries, using real measured widths
...
def _top_level_equals_positions(s):
    ...
    ``\\left...\\right`` nesting (i.e. is a real top-level relation, not
    e.g. inside a `\\frac{...}{...}` argument or a `\\left(...\\right)`
    group -- splitting a row in the middle of either leaves an unmatched
    delimiter behind, a LaTeX error)."""
    depth = 0
    positions = []
    i = 0
    n = len(s)
...
def split_progression_row(row):
    """Split one ``align`` row at each top-level ``=`` beyond the first
    into multiple ``&=``-prefixed rows -- turning a single-line
    ...
    Only ever splits at a step boundary, so it can't cut off midway
    through a fraction or other braced group.

    :returns: a list of row fragments (a 1-element list, i.e. `[row]`
        unchanged, if there's nothing to split -- 0 or 1 top-level `=`)
    """
    stripped = row.strip()
...
def _top_level_term_positions(s):
    ...
    ``\\cdot -52.547``) -- outside any ``{...}``/``\\left...\\right``
    nesting. A ``+``/``-`` counts as binary if the last significant token
    before it is something that can end a term (a digit, letter, ``}``, or
    closed ``\\right...``); anything else (start of string, another
    operator, an open ``(``/``{``) makes it a leading/unary sign instead.
    """
    depth = 0
    positions = []
    ...
    while i < n:
        ...
        if s.startswith("\\right", i):
            depth -= 1
            i += len("\\right")
            # consume the delimiter itself (e.g. ")", "]", "\}") as part
            # of this token -- crucially, *before* setting
            # prev_ends_operand, since otherwise the delimiter character
            # would fall through to the generic branches below and reset
            # it back to False, making the binary "-"/"+" that actually
            # follows a "\right)" (very common: "...\right) - {m}_1...")
            # look like a unary sign and get skipped as a split point.
            i += 2 if s.startswith("\\", i) else 1 if i < n else 0
...
def _split_at_positions(s, positions):
    """Split `s` into fragments starting at each index in `positions`
    (each fragment keeps its own leading character(s), so
...
def _top_level_cdot_positions(s):
    ...
    ``{...}``/``\\left...\\right`` nesting) -- the fallback split point
    for a term with no top-level ``+``/``-`` of its own (e.g. a single,
    very wide product chain like ``a \\cdot (\\text{...}) \\cdot b``)."""
    depth = 0
    positions = []
    i = 0
    n = len(s)
...
def _row_terms(row):
    ...
    :func:`_top_level_term_positions`); each fragment keeps its own
    leading operator/spacing, so ``"".join(terms) == row``.

    :returns: a 1-element list, `[row]`, if there's no top-level +/- to
        split at
    """
...
def _block_lhs(raw_rows):
    """The text before the first row's first top-level ``&`` in an
    ``align`` block's raw (``\\\\``-split, pre-:func:`split_progression_row`)
    rows -- its column-1 content in amsmath's 2-column layout, which every
    row in the block shares the width of (even a continuation row with
    nothing of its own before ``&``) -- or ``""`` if the first row has no
    ``&`` at all."""
...
def _iter_align_blocks(nb):
    """Yield ``(lhs, raw_rows)`` for every ``align`` block in `nb` --
    every markdown cell's source and every code cell's ``text/markdown``
    output -- where `raw_rows` are the block's ``\\\\``-split rows (before
    :func:`split_progression_row`) and `lhs` is its shared column-1
    content (see :func:`_block_lhs`)."""
...
def _all_contiguous_concats(pieces):
    ...
    ``0 <= i < j <= len(pieces)`` -- i.e. every combination
    :func:`_pack_greedy` could plausibly consider putting on one line."""
    n = len(pieces)
...
def collect_width_measurement_requests(nb):
    ...
    decide how :func:`convert_notebook_long_rows` should wrap a row: each
    ``align`` block's shared column-1 content (see :func:`_block_lhs` --
    every row in the block, even an empty-column-1 continuation row,
    loses this much width to it once amsmath lays out the 2-column
    environment), and, for each of its rows, every contiguous
    concatenation of its top-level +/- terms (see :func:`_row_terms`) --
    covering the full row itself, each individual term, and every
    combination :func:`_pack_greedy` might consider putting on one line,
    all measured directly rather than approximated by summing
...
def measure_latex_widths(snippets, engine, margin="0.75in", cwd="."):
    ...
    ``\\settowidth``, plus the page's ``\\textwidth`` under `margin` (the
    same margins :func:`~kilojoule.export.export_pdf` applies to the real
    document, via :func:`patch_margins`) -- so wrapping decisions can be
    based on actual TeX metrics instead of a character/term-count proxy.
    ...
    with no correction needed beyond it;
    :func:`collect_width_measurement_requests` measuring real term
    *combinations* directly, rather than approximating a combined width by
    summing pieces measured individually, accounts for the actual
    remaining gap.

    A snippet that fails to compile on its own (e.g. one produced by row/term
    splitting produced with unbalanced delimiters -- shouldn't happen, but
    this is a safety net) is simply missing from the returned dict rather
    than aborting the whole measurement pass; :func:`wrap_row_using_widths`
    treats a missing width as "leave this row alone".

    :param snippets: LaTeX math snippets to measure
    :param engine: LaTeX engine executable to compile with (e.g. `"xelatex"`)
    :param margin: page margin on every side, as a LaTeX length (Default value = "0.75in")
    :param cwd: directory to compile (and clean up after itself) in (Default value = ".")
    ...
    for i, snippet in enumerate(snippets):
        # Every kilojoule row has exactly one "&" (its align column
        # marker, right before the "="), at the very start for a
        # continuation row or in the middle of an opening "LHS &= ..."
        # row -- either way, "&" is only valid as an align/tabular column
        # separator, and is a hard LaTeX error ("Misplaced alignment tab
        # character") in the plain $...$ this snippet gets measured in,
        # which silently truncates (rather than fails outright) under
        # nonstopmode, corrupting the measured width. Since it renders as
        # a zero-width alignment point in the real align row anyway (not
        # part of the content's own visual width), just drop it before
        # measuring -- while still storing the result under the
        # snippet's original (unstripped) text below, so callers'
        # lookups by the original text still find it.
...
# A small residual buffer for whatever _pack_greedy can't measure
# directly -- mainly the boundary between a \cdot-expanded term and its
# top-level neighbors (see wrap_row_using_widths), which isn't among the
# combinations collect_width_measurement_requests pre-measures. At the
# cost of occasionally wrapping a row slightly before it strictly needs
# to.
_WIDTH_SAFETY_MARGIN_PT = 5.0

# amsmath's horizontal gap between align's two columns (2*\arraycolsep by
# default) -- not exposed as a single documented length, and not
# practical to measure directly: the narrowing it causes only shows up
# once a *second* row shares the block's column-1 width (see
# _block_lhs), which a single measured row can't exercise on its own. So
# it's approximated with this constant rather than measured.
_INTERCOLUMN_GAP_PT = 10.0


def _pack_greedy(pieces, widths, safe_width):
    """Greedily accumulate `pieces` onto as few lines as possible without
    a line's cumulative width exceeding `safe_width`, extending each line
    as far as the *real measured width of that exact combination*
    (``widths["".join(pieces[i:j])]``, from
    :func:`collect_width_measurement_requests` via
    :func:`measure_latex_widths`) allows -- not an approximation summed
    from individually-measured pieces, which loses the real inter-piece
    kerning and so can undershoot the true combined width. Stops
    extending (rather than guessing) at a combination that wasn't
    pre-measured; this only ever *combines* pieces, never splits one
    further, so a piece wider than `safe_width` on its own (or with an
    unmeasured combination) still ends up alone on its own line.
...
def wrap_row_using_widths(row, widths, row_width):
    """Break one ``align`` row (already past :func:`split_progression_row`)
    onto multiple continuation lines, greedily packing pieces by their
    real measured width (from :func:`measure_latex_widths`, via
    :func:`_pack_greedy`) rather than a character/term-count proxy.
    Splits at top-level ``+``/``-`` term boundaries first (see
    :func:`_row_terms`); a term that's still too wide standalone is
    further broken at its top-level ``\\cdot`` boundaries (see
    :func:`_top_level_cdot_positions`) -- the fallback for a single, very
    wide product chain with no ``+``/``-`` to split at all.

    Leaves `row` unchanged if its width (or `row_width`) wasn't
    successfully measured, if it already fits within `row_width` (less
    :data:`_WIDTH_SAFETY_MARGIN_PT`), or if it has no split point at any
    level.

    :param row_width: usable width of one numbered ``align`` row, in
        points, from :func:`measure_latex_widths`
    ...
    # Expand any +/- term that's still too wide on its own into its
    # \cdot sub-pieces, so the greedy pack below can break it up too.
    expanded = []
...
def convert_long_rows_to_multiline(text, widths, row_width):
    """Rewrite every ``\\begin{align}...\\end{align}`` block in `text`:
    split any row with a multi-step symbolic-=-substituted-=-numeric
    progression (see :func:`split_progression_row`) onto its own
    ``&=``-aligned row per step, then wrap any row still too wide at its
    ``+``/``-`` term boundaries by real measured width (see
    :func:`wrap_row_using_widths`) -- so what was one very wide line
    becomes several narrower ones either way, whether it was wide from
    chaining several `=` steps or from one expression with many
    ...
    :param widths: `{snippet: width_pt}`, from :func:`measure_latex_widths`
    :param row_width: usable width of one numbered ``align`` row, in
        points, from :func:`measure_latex_widths`
    ...
    def _replace(m):
        ...
        for raw_row in raw_rows:
            ...
            for sr in split_rows:
                ...
                # content -- everything else, including later pieces
                # split out of that same first row, is a continuation.
...
def convert_notebook_long_rows(nb, widths, row_width):
    """Apply :func:`convert_long_rows_to_multiline` to every markdown
    cell's source and every code cell's ``text/markdown`` output in `nb`
    (mutates `nb` in place).

    :returns: total number of rows split
    """
    n_split = 0
    for cell in nb.get("cells", []):
        if cell.get("cell_type") == "markdown":
...
def ensure_blank_lines_before_lists(nb):
    ...
    run-on paragraph (each ``- `` marker becomes a literal ``-`` in the
    text) instead of becoming an ``itemize``/``enumerate``. This is exactly
    why the same source renders as a proper list in the HTML export but
    not the PDF one.

    :returns: number of list starts a blank line was inserted before
    """
    n_fixed = 0
    for cell in nb.get("cells", []):
        if cell.get("cell_type") != "markdown":
...
def fix_notebook_tables(in_path, out_path, split_long_rows=False, engine=None, margin="0.75in"):
    ...
    `tabular` block (landed in ``text/markdown`` either way, so
    nbconvert's LaTeX exporter -- which doesn't consider ``text/html`` at
    all -- actually sees it), repair any output with invalid nbformat
    JSON (see
    :func:`~kilojoule.export.sanitize_notebook_outputs`), extract markdown
    `<img src="attachment:...">` images to real files (see
    :func:`extract_cell_attachments`), convert
    `kilojoule.plotting.PropertyPlot.show()`'s captioned
    `<figure>`/`<figcaption>` output to a centered, captioned
    `\\includegraphics` (see :func:`extract_cell_figures`), optionally
    measure and split long multi-step equation rows onto multiple lines
    (see :func:`measure_latex_widths` and
    :func:`convert_notebook_long_rows`), insert blank lines before
    Markdown lists that need one for pandoc to recognize them (see
    :func:`ensure_blank_lines_before_lists`), and write the result to
    `out_path`.

    :param split_long_rows: also measure and apply
        :func:`convert_notebook_long_rows` (Default value = False);
        requires `engine`
    :param engine: LaTeX engine to measure equation widths with, when
        `split_long_rows` (Default value = None)
    :param margin: page margin :func:`measure_latex_widths` assumes when
        measuring (Default value = "0.75in") -- should match whatever
        margin the caller will actually apply via
        :func:`~kilojoule.export.export_pdf`'s `patch_margins`
    :returns: `(n_tables_fixed, n_outputs_sanitized, n_attachments_extracted, n_figures_extracted, attachment_paths, n_rows_split, n_lists_fixed)`
        -- `attachment_paths` covers both attachments and figures, all
        needing the same cleanup
    """
    ...
    for cell in nb.get("cells", []):
        ...
        for output in cell.get("outputs", []):
            ...
            # higher-priority) representation exists too. Check both
            # keys for a table (preferring `text/markdown` if both
            # happen to have one), but always land the fixed LaTeX in
...
def patch_cancel_package(tex_path):
    ...
    compile) and ``\\usepackage{capt-of}`` (so :func:`html_table_to_latex`
    can caption a state table via ``\\captionof`` without it being a
    floating ``table`` environment) to a generated `.tex` file."""
    content = tex_path.read_text(encoding="utf-8")
    anchor = r"\usepackage{amsmath} % Equations"
    if anchor not in content:
        warnings.warn(
            "export_pdf(): could not find the expected amsmath line to "
            "patch \\usepackage{cancel}/\\usepackage{capt-of} after -- "
            "nbconvert's LaTeX template may have changed. \\cancel{} "
            "terms (if any) will fail to compile, and state table "
            "captions will fail to compile too."
        )
        return
    replacement = (
...
def patch_table_captions(tex_path):
    """Re-enable normal caption rendering for :data:`_TABLE_CAPTION`
    (table) and :func:`html_figure_to_latex` (figure) captions
    specifically, in a generated `.tex` file.
    ...
    ``\\includegraphics``/``\\adjustimage`` calls for any other embedded
    image -- they never invoke ``\\caption``/``\\captionof`` at all, so
    the format they'd use is moot.
    """
    content = tex_path.read_text(encoding="utf-8")
    anchor = r"\captionsetup{format=nocaption,aboveskip=0pt,belowskip=0pt}"
    if anchor not in content:
        warnings.warn(
            "export_pdf(): could not find the expected \\captionsetup "
            "line to patch a table-/figure-specific override after -- "
            "nbconvert's LaTeX template may have changed. State table "
            "and figure captions will be blank."
        )
        return
    replacement = (
        anchor + "\n    "
...
def remove_title_block(tex_path):
    ...
    if new_content == content:
        warnings.warn(
            "export_pdf(): could not find \\maketitle to remove -- "
            "nbconvert's LaTeX template may have changed. The PDF will "
            "still show the generic title/date."
        )
        return
...
def patch_margins(tex_path, margin="0.75in"):
    ...
    if n == 0:
        warnings.warn(
            "export_pdf(): could not find the expected \\geometry{...} "
            "call to patch margins on -- nbconvert's LaTeX template may "
            "have changed. Margins will be left at their default."
        )
        return
    tex_path.write_text(new_content, encoding="utf-8")
...
# actual bullet item, emits it as a whole separate one-item `enumerate`
# per bullet rather than a single flat list across all of them. Each
# mini-enumerate sets its own \labelenumi once (redundantly, every time,
# to the same definition) and its own \setcounter (omitted -- defaulting
# to 0, i.e. "a"/"1" -- only for the very first one).
...
def _try_nested_lettered_list(body, item_re, label_opt):
    ...
    pattern (see :data:`_NESTED_LETTER_ITEM_RE`) into one flat
    ``enumerate``, verifying every item's own counter is exactly
    0, 1, 2, ... in order (the omitted-``\\setcounter`` first item counts
    as 0) -- i.e. that pandoc really did parse a strictly sequential
    ``(a)``/``(b)``/... (or ``1)``/``2)``/...), not e.g. ``(a)``, ``(c)``.

    :returns: the replacement ``enumerate`` text, or `None` if `body`
        doesn't consist entirely of this nested pattern in sequence
    """
...
def convert_lettered_lists(tex_path):
    ...
    sequential ``(a)``/``(b)``/``(c)...`` or ``1)``/``2)``/``3)...`` label
    into a proper LaTeX ``enumerate`` (via `enumitem`'s ``label=`` option),
    stripping the now-redundant literal label text from each item.
    ...
    inside each outer bullet item (see
    :func:`_try_nested_lettered_list`); both are handled here.

    :returns: number of lists converted
    """
    content = tex_path.read_text(encoding="utf-8")
    n_converted = 0
    ...
    for i in range(passes):
        try:
            result = subprocess.run(
                ...
                # xelatex/lualatex/pdflatex write UTF-8 (unit symbols like
                # deg, math glyphs from kilojoule's LaTeX output, etc.) to
                # their console log regardless of platform. Without an
                # explicit encoding, Windows decodes subprocess text output
                # with the console's ANSI codepage (e.g. cp1252), which
                # can't represent most of that and raises
                # UnicodeDecodeError inside subprocess's own reader thread
                # -- silently turning result.stdout/stderr into None rather
                # than raising here, so this pass fails much more
                # confusingly, in "log = result.stdout + result.stderr"
                # below. errors="replace" keeps the log readable even if
                # a byte is genuinely undecodable.
